# Haute & Hazard v7.9 — Beginner Mode

Beginner Mode is the recommended first game. It uses the normal v7.9 Queens, starter decks, Wardrobe Rack, Thrift Store, Penalties, **Tip Count**, Shopping economy, Look rules, Slay rules, and Dragdagulan†.

The only simplification is how you read each Stage.

> **† Legal/IP review:** `Dragdagulan` is working prototype terminology credited to *Drag Den Philippines* and remains subject to legal/IP, trademark, and publisher review.

## Use these Stage fields

1. **Favored Tenet**
2. **Featured Brand**
3. **Slay Target**
4. **Reward**
5. **Venue Effect**

## Ignore these advanced Stage fields

- Judge
- Spotlight Requirement
- Judge's Favor
- Brand Ovation

No cards need to be removed or reprinted.

## Everything else stays active

Players still use:

- Tenets and Brands
- Face / Wig / Body / Shoes Coordinates
- Matching Look
- Perfect Illusion
- Fusion
- **Tip Count**
- Shopping
- Appeal
- Slay
- LS and Dragdagulan†
- Penalties
- Queen Signature Abilities
- once-per-game Special Appeals
- Gross SP and the normal end trigger

## Recommended teaching setup

For the easiest first game, use **2 players**.

Each player receives:

- 7 × Basic Beat — printed Tip 1
- 3 × Messy Lip Sync — printed Tip 0
- 2 × Chapstick — printed Tip 1

Shuffle the 12 venue Stages and point out only the five Beginner Mode fields.

## v7.9 turn structure

0. **Tip Count** — add printed Tips from every card currently in hand. Count each card once.
1. **Transformation**
2. **The Reveal**
3. **Shopping**
4. **Slay / Dragdagulan† / Pass**
5. **Cleanup**

A card counted during Tip Count can still be played/equipped that turn. Its printed Tip value does not trigger again merely because it was played.

## Slaying a Stage

To Slay:

1. meet or exceed the printed Slay Target;
2. satisfy any normal requirement outside the ignored advanced layer;
3. claim the Stage trophy;
4. gain the printed reward;
5. resolve Slay effects and Curtain Call.

Do not check Spotlight Requirement or award Judge's Favor/Brand Ovation in Beginner Mode.

## Moving to full Stage rules

After players understand the core loop, re-enable:

- Judge
- Spotlight Requirement
- Judge's Favor
- Brand Ovation

Nothing else changes.

## Teaching line

> **Count your Tips, build the Look, Shop the Wardrobe, and Slay the venue. The Judge layer comes next game.**

**USE:** Favored Tenet • Featured Brand • Slay Target • Reward • Venue Effect  
**IGNORE:** Judge • Spotlight Requirement • Judge's Favor • Brand Ovation

See [STARTING_DECKS.md](STARTING_DECKS.md), [CURRENT_GAMEPLAY.md](CURRENT_GAMEPLAY.md), and [releases/v7.9/STAGE_VENUE_LIST.md](../releases/v7.9/STAGE_VENUE_LIST.md).
